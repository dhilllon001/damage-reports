...
export function ReportsPage() {
  ...
  return (
    <AppShell search={search} onSearch={(v) => { setSearch(v); setPage(0) }} onNew={() => setNewOpen(true)}>
      ...
      <section className="mb-5 grid grid-cols-2 gap-2.5 sm:grid-cols-4 xl:grid-cols-8">
        {stats.map((s) => (
          <button
            key={s.label}
            type="button"
            ...
            className="rounded-[16px] bg-card px-3.5 py-3.5 text-left shadow-[var(--shadow-rest)] transition hover:shadow-[var(--shadow-hover)]"
          >
            <div className="text-[22px] font-extrabold tracking-[-0.04em] text-ink">{s.value}</div>
            <div className="mt-0.5 text-[10.5px] font-semibold uppercase tracking-[0.06em] text-ink-3">
              {s.label}
            </div>
          </button>
        ))}
      ...
      <section className="mb-4 space-y-3 rounded-[18px] bg-card/80 p-4 shadow-[var(--shadow-rest)] backdrop-blur">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-2">
            <div className="text-[11px] font-semibold uppercase tracking-[0.06em] text-ink-3">Unit Type</div>
            <Segment
              value={unitType}
          ...
          <div className="space-y-2">
            <div className="text-[11px] font-semibold uppercase tracking-[0.06em] text-ink-3">Category</div>
            <Segment
              value={category}
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
          <div className="space-y-2">
            <div className="text-[11px] font-semibold uppercase tracking-[0.06em] text-ink-3">Severity</div>
            <Segment
              value={severity}
          ...
          <div className="flex flex-wrap gap-2">
            <select
              ...
              className="h-9 rounded-full border border-line bg-white px-3.5 text-[12.5px] font-medium text-ink-2 outline-none focus:border-accent"
            >
      ...
      {/* Desktop table */}
      <section className="hidden overflow-hidden rounded-[18px] bg-card shadow-[var(--shadow-rest)] md:block">
          ...
          <table className="w-full min-w-[1100px]">
            <thead>
              <tr className="border-b border-line bg-[#fafafa]">
                {[
                  ...
                  <th
                    key={h}
                    className="px-4 py-3.5 text-left text-[11px] font-semibold uppercase tracking-[0.05em] text-ink-3"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {slice.map((r) => (
                <tr
                  key={r.id}
                  className="border-b border-line/80 transition hover:bg-[#f7f8fa]"
                >
                  ...
                  <td className="px-4 py-3.5">
                    <div className="text-[12.5px] font-semibold text-ink">{r.unitType} {r.unitNo}</div>
                    <div className="text-[11px] text-ink-3">{r.source}</div>
                  </td>
                  <td className="px-4 py-3.5 text-[12.5px] text-ink-2">{r.liableParty ?? '—'}</td>
                  <td className="px-4 py-3.5">
                    <Badge tone={severityTone(r.severity)}>{r.severity}</Badge>
                  </td>
                  <td className="max-w-[220px] px-4 py-3.5 text-[12px] text-ink-2">
                    <span className="line-clamp-2">{r.location}</span>
                  </td>
                  <td className="px-4 py-3.5">
                    <Badge tone={statusTone(r.status)}>{r.status}</Badge>
                  </td>
                  <td className="px-4 py-3.5 text-[12.5px] font-semibold text-ink">
                    {r.amount != null
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        onClick={() => navigate(`/reports/${r.id}`)}
                        className="rounded-full p-2 text-ink-3 hover:bg-black/[0.04] hover:text-ink"
                        aria-label="More"
                      >
      ...
      <section className="space-y-3 md:hidden">
        {slice.map((r) => (
          <article
            key={r.id}
            className="rounded-[18px] bg-card p-4 shadow-[var(--shadow-rest)]"
          >
            ...
            <div className="mb-3 space-y-1 text-[12.5px] text-ink-2">
        ...
        <div className="rounded-[18px] bg-card shadow-[var(--shadow-rest)]">
          <Pagination
            from={from}
            to={to}
            total={filtered.length}
    ...
    <div className="flex flex-col gap-3 border-t border-line px-4 py-3.5 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex items-center gap-3">
        <select
          ...
          className="h-8 rounded-full border border-line bg-white px-3 text-[12px] outline-none"
        >
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className={cn(
              'rounded-full p-1.5 text-ink-2 hover:bg-black/[0.04] disabled:opacity-30'
            )}
          >
            <ChevronLeft size={16} />
          </button>
          <span className="min-w-[72px] text-center text-[12.5px] font-medium text-ink-2">
            {page + 1} / {pageCount}
          </span>
          <button
            disabled={page >= pageCount - 1}
            onClick={() => onPage(page + 1)}
            className="rounded-full p-1.5 text-ink-2 hover:bg-black/[0.04] disabled:opacity-30"
          >
